...
public class RecruitFilterSheet extends BottomSheetDialogFragment {
    ...
    public void onViewCreated(@NonNull View root, @Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            if (s instanceof RecruitFilters) {
                current = (RecruitFilters) s;
            }
        }
        if (current == null) {
        ...
        // 모집유형
        selectRecruitTypeChip(current.recruitType);
        ...
        btnSkillMax.setOnClickListener(v -> {
            showPopup(btnSkillMax, skillItems, selected -> {
                ...
                current.skillMax = "전체".equals(selected) ? null : Integer.parseInt(selected);
            });
        });

        // 모집유형
        chipRecruitTypeAll.setOnClickListener(v -> selectRecruitTypeChip("전체"));
        ...
        // 포지션 토글
        chipPosAll.setOnClickListener(v -> selectPositionChip("전체"));
    ...
    // ====== 팝업 ======
    private interface OnPopupItemSelected {
    ...
    private void showTenMinuteTimePicker(boolean from) {
        ...
        final android.widget.NumberPicker pickerHour = new android.widget.NumberPicker(requireContext());
        pickerHour.setMinValue(0);
        pickerHour.setMaxValue(23);
        pickerHour.setFormatter(i -> String.format(Locale.getDefault(), "%02d", i));
        pickerHour.setValue(initHour);

        final android.widget.NumberPicker pickerMinute = new android.widget.NumberPicker(requireContext());
    ...
    private void selectRecruitTypeChip(String type) {
        current.recruitType = type;
        clearChip(chipRecruitTypeAll);
        clearChip(chipRecruitTypeRegular);
        clearChip(chipRecruitTypeMerc);
        if ("전체".equals(type)) setChip(chipRecruitTypeAll);
        else if ("regular".equals(type)) setChip(chipRecruitTypeRegular);
        else if ("mercenary".equals(type)) setChip(chipRecruitTypeMerc);
    }

    // ====== 포지션 복원용 ======
    private void restorePositionChipsFromCurrent() {
        clearChip(chipPosAll);
        clearChip(chipPosGK);
        clearChip(chipPosDF);
        clearChip(chipPosMF);
        clearChip(chipPosFW);
        ...
        for (String p : parts) {
            String v = p.trim();
            switch (v) {
                case "GK": setChip(chipPosGK); break;
                case "DF": setChip(chipPosDF); break;
                case "MF": setChip(chipPosMF); break;
                case "FW": setChip(chipPosFW); break;
            }
        }
    }
    ...
    private void selectPositionChip(String pos) {
        ...
        if (selected.isEmpty()) {
            ...
            current.position = TextUtils.join(",", selected);
        }
